...
func Create(product entities.Product) bool {
	result, err := config.DB.Exec(`INSERT INTO products(name,category_id,description,stock,created_at,updated_at) VALUES (?,?,?,?,?,?) `, product.Name, product.Category.Id, product.Description, product.Stock, product.CreatedAt, product.UpdatedAt)
